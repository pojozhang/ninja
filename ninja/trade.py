...
from utils import print_json, parse_args, to_dict
...
def build_context(cron, args):
    trader = okex.Client(config=okexConfig, headers=okexHeadersConfig)
    return Context(cron=cron, args=args, trader=trader)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cron', dest='cron', type=str, default='forever',
                        help='crontab')
    ...
    context = build_context(args.cron, to_dict(strategy_args))
